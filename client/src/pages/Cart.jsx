...
import { assets, dummyAddress } from '../asset/assets';
import { useAppContext } from '../context/AppContext'
import toast from "react-hot-toast";

export const Cart = () => {
    const { product, currency, cartItems, removeFromCart, getCartCount, updateCartItem, navigate, getCartAmount, axios, user, setCartItems } = useAppContext();
    ...
    const placeOrder = async () => {
        try {
            ...
            if (paymentOption === "COD") {
                ...
                const { data } = await axios.post('/api/order/stripe', { userId: user._id, items: cartArray.map(item => ({ product: item._id, quantity: item.quantity })), address: selectedAddress._id })
